...
**[Terminology fixed 2026-09-13, user-caught — precise end-states, not
one collapsed "CLOSED."]** `SOLVED` / `DISSOLVED` / `EXHAUSTED` /
`BLOCKED` / `STOPPED` are different conditions and must not be used
interchangeably: `DISSOLVED` means the question itself had no content
(#3); `EXHAUSTED` means a bounded search campaign ran out, the
underlying physical question may still be open (#3's Fisher-line, #4);
`STOPPED` means an active decision to not continue, not a proof there
is nothing to find (#2); `CLOSED/BOUNDED` means a specific,
precisely-scoped question was answered within its own stated
operationalization — never read as "MULTING is physically established"
(#1). None of these four rows says anything about MULTING's own
causal/cosmological correctness (`NON-IDENTIFIED` throughout,
`NO_AUTHOR_ERROR`).

| # | Bottleneck | End-state | Status | What would move it |
|---|---|---|---|---|
| 1 | F→H_MULT(z) bridge | **CLOSED/BOUNDED** (within current derivation/operationalization) | **CLOSED 2026-09-12/13 (`FINDING_P223`, commit `e421126`), superseding every row below.** `P223` embedded v82's own literal force law into v82's own kinematic-translation machinery (H1, symbolically confirmed: `k_A(z)`-dependent terms survive, no internal cancellation) and independently reproduced this project's own `docs/127` `G_αβ=0` result exactly for the same power-law shapes (H2). Synthesis (H3, narrowed after Step 8a): `docs/127`'s S-S closure and v82's single-pair bridge answer structurally different questions about a shared force-law shape (one pair's finite trajectory vs. a population's `r→∞` tail) — **not in contradiction**, under the literal-text reading. This closes `docs/153`'s own restated causal-compatibility question. A SEPARATE thread (`P158`'s own magnitude/mechanism question — which of `F^(1)`/`F^(2)` gets more population-averaging boost — unrelated to the causal-compatibility question P223 answers) ran 2026-09-11→13 via a real FLAMINGO-based estimand and is ALSO closed, for its own current operationalization — see §9. **Does NOT mean MULTING is physically established** — only that this specific derivation/compatibility question is answered within its stated scope. | Nothing pre-named — this line is closed, not stalled. Reopening either sub-question needs a genuinely new input (v82's own physical INTENT for `s(z)` carrying unstated population content is explicitly out of `P223`'s own scope, per its Skeptic Objection 2), not a re-attempt of the same literature-search pattern the row below once named |
| 2 | Unique completion | **STOPPED / UNRESOLVED** (active decision not to continue, not a proof of nothing-to-find) | Untouched by the `P132`-`P139` round, `docs/134`. **2026-09-07:** `/hypothesis-arbiter` on the dipole's ontology (`H_body` / `H_fluid` / `H_avg`) returned only the LEAST falsifiable survivor and recommended **stopping** structural computation — the arbiter's own read was that further iteration here did not clear an expected-value-of-information (EVI) bar, not that the question was answered. Recommendation taken. | A genuinely new input, not another variant |
| 3 | Absolute scale / observable mapping | **DISSOLVED** (the `(A,g,κ)` question, `P206`) **+ EXHAUSTED** (the separate `P191`-`P194` Fisher-forecast line on v82's own `(β1,β2)`) | `(A,g,κ)` **DISSOLVED 2026-09-07 (`FINDING_P206`)** — it was never a measurement gap. `P133`'s rank-2 result IS the one-dimensional **field-normalization redundancy** `FINDING_P52` had already derived on 2026-08-16; `P133` renamed it 8 days later and made it a bottleneck without connecting the two. `L := 2a−b−c` is the field-rescaling weight, the null direction `(−1, ½, ½)` is the `φ→λφ̄` generator, so rank 2 is **complete**: 3 coordinates, 1 redundancy, 2 physical d.o.f., all determined. Step 8a run twice with reworded prompts, both CONFIRMED-REAL. The SEPARATE `P191`-`P194` Fisher-forecast line (v82's own `(β1,β2)` degeneracy — a different object, Gate 1) is **EXHAUSTED**, not dissolved: it maps a real, quantified, in-principle-buildable path (DONE 2026-09-05, §5) with no next candidate named since — the underlying physical question (does a real high-z `H(z)` measurement exist at the needed precision) remains genuinely open, only this line's own campaign is spent. | `(A,g,κ)`: nothing — the question as posed had no solution and needed none. **What remains open is `P52`'s own KG2**, answered as far as data allows on 2026-09-07 (`P208`/`P209`, §7). `(β1,β2)` Fisher-line: a genuinely new question (not another z-set variant) or real high-z data actually materializing |
| 4 | IC-sensitivity | **SEARCH CAMPAIGN EXHAUSTED** (physical question not necessarily resolved) | CLOSED — 5 mechanism candidates excluded (`P135`/`P136`/`P138`/`P139`/`P141`), synthesis found no new candidate. The physical question (is the growth-channel discriminator IC-sensitive at low `k`) stays genuinely open — only the campaign on this information set is spent. | A genuinely new mechanism class, not a 6th variant of the 5 already excluded |
