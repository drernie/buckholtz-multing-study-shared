...
| `zenodo_21204955_supplemental/` | **[NEW, 2026-09-02]** TJB's own supplemental code+data archive for v82 (Zenodo record 21204955, `isSupplementedBy` relation to the manuscript's own 22004287) — the author's canonical force-law implementation (`code/multing_core.py`), every fixed numerical input with `data`/`theory`/`definition` provenance tags (`code/assumptions.yaml`), the raw 31-point Cosmic Chronometer + SH0ES + DESI DR2 data (`data/*.csv`), all 7 rows of Table II reproduced and independently re-verified by a genuinely separate AI session (`code/regenerate_prompt.md`, `process_documentation/`), and a 20-day author-AI development transcript. **Not Table A1** — a different, author-authored artifact; see the archive's own `INDEX.md` for the explicit non-transfer note. Full contents + key extracted facts (accretion-force term, T0 tension, H0-anchor-circularity gap relevant to bottleneck 3): `zenodo_21204955_supplemental/INDEX.md`. |
