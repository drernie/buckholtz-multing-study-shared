...
|---|---|---|
| **1 — C1 closure** (docs/133) | `NEEDS_DATA`, leaning `PIVOT` | The chain {k_A,r_A,D(z),β_d,β_q}→F_d/F_m→H_MULT(z) does not reproduce from public materials — not specified, not reproducible, not independently checkable, blocks Planck/Euclid tests. **This is the main result of the whole trilogy.** Strengthened 2026-07-23, split into two evidentiary tiers per user correction: **M1 (CONFIRMED, unconditional)** — Table A1's H_MULT correlates r≈0.9996 with H_FLRW, ≈1.074× rescaling, no independent signal, from the raw CSV alone. **M2 (originally "robust direction, conditional magnitude")** — **downgraded to illustrative-only** after the source-fidelity gate was actually run 2026-07-23 (direct read of Appendix A.1, pp.32–38 of the v6 PDF): the PDF specifies no H(z)-combination formula at all, and gives TJB's own explicit design constraint ("dipole dominates at low z, quadrupole at high z") that the June reconstruction's parameters likely violate (quadrupole found dominant at every z including z=0) — neither the ×4365 figure nor the "opposite direction" framing reliably represents TJB's actual process; removed from the letter. **New, sharper deduction from M1 alone (survives the M2 downgrade):** a near-constant rescaling cannot produce a curve that crosses ΛCDM — yet the July-20 chart's red/blue curves do cross — so Table A1 and the July chart are **likely not the same computational procedure**. Folded into letter Q1, now also citing TJB's own Appendix A.3 words ("disagreed noticeably" across services) as safer, fully author-sourced support. Q4 rewritten to ask for the missing combination formula instead of presenting a reconstruction attempt as evidence. |
