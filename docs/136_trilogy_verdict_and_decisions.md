...
|---|---|---|
| **1 — C1 closure** (docs/133) | `NEEDS_DATA`, leaning `PIVOT` | The chain {k_A,r_A,D(z),β_d,β_q}→F_d/F_m→H_MULT(z) does not reproduce from public materials — not specified, not reproducible, not independently checkable, blocks Planck/Euclid tests. **This is the main result of the whole trilogy.** Strengthened 2026-07-23, split into two evidentiary tiers per user correction: **M1 (CONFIRMED, unconditional)** — Table A1's H_MULT correlates r≈0.9996 with H_FLRW, ≈1.074× rescaling, no independent signal, from the raw CSV alone. **M2 (ROBUST DIRECTION, CONDITIONAL MAGNITUDE)** — a reconstructed bridge formula gives H(z) moving opposite to Table A1's own H_MULT; the trend is robust, the exact ×4365 figure at z=8.5 depends on reconstructed cluster inputs not yet re-checked against the source PDF (source-fidelity gate NOT YET PASSED, docs/133). **New, sharper deduction from M1 alone:** a near-constant rescaling cannot produce a curve that crosses ΛCDM — yet the July-20 chart's red/blue curves do cross — so Table A1 and the July chart are **likely not the same computational procedure**. Folded into letter Q1 (v10); Q4's exact number removed from the letter body per the same correction. |
