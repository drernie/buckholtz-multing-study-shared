...
| LABELS-01 **A** | items 11 `labels_emitted` and 13 `ambiguity_report` are multisets recorded in the order labels are raised / sets, deduplicated and written in a canonical order | **set**, deduplicated at emission, serialised byte-wise ascending. The semantic object is unordered; the array is only its deterministic serialisation, and execution order is not recoverable from it. Chronological was rejected for a second reason beyond taste: it would make the execution order of the nine controls observable, reopening a question that item 8's control-indexed array was relied on to close. Raised in round 6 by an isolated-context reviewer. | `output_schema.json` → items 11, 13 (`semantics`, `duplicates`, `row_order`) | TV22 |
| CLIP-01 **A** | `clip: [0.05, 60.0]` restricts the refined grid to the interval / clamps out-of-range nodes to the boundary, as `numpy.clip` does | **restrict**: keep the nodes satisfying `clip[0] <= mu <= clip[1]`, drop the rest, never clamp. Clamping would place one duplicate node on the boundary per out-of-range `ii`; under IDX-01 every fit increments `edge_count`, so the duplicates would be visible in a reported `int64` array. Raised in round 6; downstream of IDX-01, which is what made the difference observable. | `reference_config.yaml` → `grids.refined.clip_semantics` | TV21 |
...
| SDMC-01 **B'** | `sdmc_contract` (named by S2/S3/S4) is a dangling reference with no formula anywhere in the four files / it is v6 §10's SdMC formula, silently inherited but never spelled out where the state machine points, and its own natural consequence at `mua = 0` (case B, every bisection midpoint also rejected) was never stated as a reachable, distinct outcome | `sdmc_contract` is now a real section: the full formula, the KDE bandwidth with its `iqr=0`/`sdev=0` fallbacks, and the `mua=0` edge case made explicit as `SdMC = NOT_DEFINED` (a consequence of the general zero-Q95 rule, not a new exception). Found by a properly-scoped second reader (v6+v7+four files) after an earlier same-model reader, scoped to the four files alone, wrongly raised ~20 findings that were actually already answered in v6. | `state_machine.yaml` → `sdmc_contract` | TV23 |
...
| CTRL7-01 **B'** | when guard G3 fails, control 7's `Wt`-dependent Drel is silently absent / it emits some other sentinel / CTRL-02's "all nine emit" is violated | control 7 emits `NaN` for its Drel measured value (both `mu` probes, both models) when `Wt` cannot be formed; `NaN` reports "not computed", CTRL-02 still holds (all nine rows present) | `state_machine.yaml` → `control_completion.control_7_wt_undefined` | TV24 |
| NAN-01 **B'** | items 4 and 10's `L95`/`SdMC` float slots hold an unstated value (0.0? carried-through garbage?) when the row's state has no numeric endpoint / SHAPE-01's NaN-plus-state-array contract, already claimed for items "3, 4, 10" in this ledger's own SHAPE-01 row, was never actually written into items 4 and 10's field-level schema | items 4 and 10 gain the same `nan_where` field-level annotations item 3 already had, completing SHAPE-01 rather than adding a new rule: `NaN` where the row's state is not `FINITE_LIMIT`/`FINITE_LIMIT_NEAR_ZERO`/`FINITE_LIMIT_MULTICROSS` (`L95`) or is `NOT_DEFINED` (`SdMC`) | `output_schema.json` → items 4, 10 | TV25 |
