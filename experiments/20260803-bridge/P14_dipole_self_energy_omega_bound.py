...
def main() -> None:
    ...
    print("  unrelated to kappa (P11/P12's mu~H0/c). [CORRECTED after skeptic review:]")
    print("  this is NOT the first calculation depending on kappa's absolute magnitude")
    print("  -- FINDING_P7 and MODEL_SPEC_AUDIT already silently used kappa=1 without")
    print("  flagging it (see FINDING_P14 section 5); this is the first to NOTICE the")
    print("  problem. The printed Omega_phi number above assumed kappa~O(1) with no")
    print("  basis, but its SCALING (~kappa^2) is real and gives a conditional bound")
    print("  -- see caveat 1 below, not 'no information'.")
    print()
    print("CAVEATS [updated after skeptic review]:")
    print("  1. [LOAD-BEARING] kappa's absolute normalization is unfixed project-wide --")
    print("     NOT the first calculation to depend on it (FINDING_P7 and")
    print("     MODEL_SPEC_AUDIT already silently used kappa=1; this is the first to")
    print("     NOTICE it). Scaling IS informative: Omega_phi(kappa)~3.3e11*kappa^2,")
    print("     so Omega_phi<=1 gives kappa<=1e-6 -- CONDITIONAL on caveat 4 below.")
    ...
    print("  4. [UNRESOLVED] whether the self-energy channel computed here survives")
    print("     as a real, separate contribution is now in doubt: P9's exact exterior")
    print("     zero (this same radially-aligned configuration) forces cross terms to")
    print("     EXACTLY CANCEL the self-energy sum in the idealized continuous limit.")
    print("     Whether that cancellation persists for a realistic, discrete, finite-")
    print("     size population (this script's regularization) is NOT established")
    print("     either way -- see FINDING_P14 section 1 for the full argument.")
