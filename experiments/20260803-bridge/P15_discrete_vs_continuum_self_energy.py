...
def main() -> None:
    ...
    print("\n[REGIME C -- CONTEXT-BLIND SKEPTIC'S ALTERNATIVE LIMIT] same setup as Regime")
    print("  A (p_total and ring radius FIXED, so surface dipole density mu=p_total/")
    print("  circumference is ALREADY constant in Regime A too), but this time r_min is")
    print("  held FIXED (a genuine physical core size) instead of shrinking with N.")
    print("  This is the 'resolution of scale' vs 'physical grain size' distinction the")
    print("  skeptic raised: Regime A conflated the two by shrinking both together.")
    for n in (8, 32, 128, 512):
        p_i = p_total / n
        r_min_fixed = r_min_shrinking_base  # NOT divided by n -- the key change vs Regime A
        s, c, t = total_energy(n, ring_radius, p_i, r_min_fixed)
        nn_spacing = 2 * math.pi * ring_radius / n
        print(
            f"  N={n:4d}: self={s:+.6e}  cross={c:+.6e}  TOTAL={t:+.6e}  |total/self|={abs(t / s):.4f}"
            f"  (nn_spacing/r_min={nn_spacing / r_min_fixed:.1f})"
        )
    print("  -> self ~ n*(1/n)^2/const = 1/n -> 0; cross approaches a FINITE limit (the")
    print("     actual field energy of a fixed-thickness double layer). This is a THIRD")
    print("     distinct limit, not just 'the opposite of Regime A': at N=512 nn_spacing")
    print("     approaches r_min (ratio printed above), so the point-dipole cross-term")
    print("     approximation itself starts to strain -- flagged, not hidden. The")
    print("     qualitative lesson stands regardless: self/cross scaling under N->infinity")
    print("     depends on what is held fixed (r_min shrinking vs r_min fixed), so THREE")
    print("     limits exist (A, B, C), not two -- 'category mismatch' is real but was")
    print("     under-demonstrated by only showing A and B.")
    ...
    print("Regime A (r_min shrinks with N)    : DIVERGES (self~N^2). [CORRECTED after")
    print("                                     skeptic review:] the ring geometry never")
    print("                                     had P9's exact-zero property to begin with")
    print("                                     (shell theorem is a 2D/3D symmetric-shell")
    print("                                     result, not a 1D-ring one) -- this shows")
    print("                                     that shrinking r_min with N is a distinct,")
    print("                                     divergent idealization, not a direct")
    print("                                     'revealing of a mismatch with P9' itself.")
    print("Regime C (r_min FIXED, mu fixed)   : self->0 as 1/N, cross->finite -- the")
    print("                                     OPPOSITE scaling of Regime A from the same")
    print("                                     starting setup, differing only in whether")
    print("                                     r_min shrinks with N. Confirms self-vs-")
    print("                                     cross scaling genuinely depends on which")
    print("                                     quantity is held fixed: THREE limits (A,")
    print("                                     B, C), not a simple two-way split.")
    print("Regime B (realistic, fixed size)   : |total/self| stays at 1.0000 (4dp) across")
    print("                                     N=8..128. [CORRECTED after skeptic review:]")
    print("                                     actual cross/self ratio is ~4e-5, NOT ~1e-9")
    print("                                     -- the 1e-9 figure in an earlier draft was")
    print("                                     Regime A's ratio, misattributed to Regime B.")
    print("                                     Cross is still utterly subdominant (~4e-5),")
    print("                                     just not by as many orders as first stated.")
    print("SCOPE LIMIT: a ring (1D loop) is a simplified stand-in for the full 2D")
    print("  spherical shell P9 computed exactly -- the ring does NOT itself satisfy P9's")
    print("  shell theorem regardless of regularization, so no regime here literally")
    print("  reproduces P9's cancellation. The qualitative self-vs-cross scaling behaviour")
    print("  should transfer to 2D, but that transfer is asserted, not shown, and the")
    print("  specific numeric ratios are ring-geometry-specific.")
