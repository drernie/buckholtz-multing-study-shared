...
def main():
    ...
    # ------------------------------------------------------------------
    print("\n" + "-" * 78)
    print("ADDENDUM #4 (2026-08-18) -- propagates FINDING_P59's linear-order")
    print("nu=0/nu=1 results into THIS file's own Part 4/Part 5. NOT new")
    print("physics -- FINDING_P59 already did the Route-B (rho_phys) tensor")
    print("construction and its own context-blind skeptic review; this section")
    print("re-verifies the PROPAGATION into P56's own equations is stated")
    print("correctly, reusing P56's own already-computed quantities directly")
    print("wherever possible (not re-running P59's heavier machinery). This")
    print("file's shared rhobar/deltarho symbols ARE rhobar_A/delta_rho_A in")
    print("FINDING_P59's more precise later naming -- established already by")
    print("FINDING_P58's own retraction (Addendum #3), not asserted new here.")
    print("-" * 78)
    print()
    print("  WHY Part 4/Part 5 above need superseding, not just citing: BOTH")
    print("  equations above assumed the closure nabla_mu[T_phi+T_m(rhobar)]=0")
    print("  applies to T_m built DIRECTLY from rhobar (bare, per FINDING_P58).")
    print("  But FINDING_P33 already establishes rho_phys=rhobar*(1-g_hat*")
    print("  phibar) is the ACTUAL physical, gravitating mass density -- the")
    print("  physically meaningful closure is on T_m(rho_phys), not T_m(rhobar)")
    print("  directly. FINDING_P59 tested exactly this (Route B) and found the")
    print("  corrected forms below.")
    print()
    print("  [Corrected continuity, nu=0] -- self-contained proof, reusing")
    print("  Part 4's OWN already-computed div_Tm_0 (T_m(rhobar)'s own")
    print("  divergence): div_Tm_0 contains NO g_hat at all (T_m's kinematic")
    print("  construction never used g_hat -- already implicit in Part 4's own")
    print("  g_hat=0 check, since that check left div_Tm_0 itself unchanged).")
    print("  Confirming this directly (not merely inferring it):")
    check_continuity_general = sp.simplify(div_Tm_0 - expected_decoupled)
    assert check_continuity_general == 0, (
        "div_Tm_0 (general, not restricted to g_hat=0) does not match the "
        "simple uncoupled continuity form -- Addendum #4's claim that "
        "div_Tm_0 is g_hat-independent is wrong, do not report the correction"
    )
    print(f"    div_Tm_0 (general) == {expected_decoupled}  [g_hat-independent, CONFIRMED]")
    print("  FINDING_P59's own Route-B comparison (not reproduced here --")
    print("  requires the rho_phys tensor construction, absent from this")
    print("  file) shows [div_Tm_0(rho_phys)-Q^0_A], on rhobar's own on-shell")
    print("  relation, factors EXACTLY as (1-g_hat*phibar)*div_Tm_0(rhobar) --")
    print("  i.e. (1-g_hat*phibar) times THIS file's own div_Tm_0, just shown")
    print("  g_hat-independent above. Since (1-g_hat*phibar) is generically")
    print("  nonzero, the PHYSICAL closure (on T_m(rho_phys), not T_m(rhobar))")
    print("  forces div_Tm_0(rhobar)=0 -- NOT div_Tm_0(rhobar)=Q^0 as Part 4")
    print("  above assumed. Part 4's original closure equation is thus")
    print("  SUPERSEDED (not merely re-derived in a different variable) by:")
    print(f"    {expected_decoupled} = 0")
    print("  the SIMPLE, UNCOUPLED continuity equation on delta_rho/V_x -- no")
    print("  g_hat source at all. Matches FINDING_P59's own nu=0 result exactly.")
    print()
    print("  [Corrected Euler, nu=1, W-variable] -- Part 5 above's V_x-form is")
    print("  RELABELED Route-A-only (an exact statement, but valid only if")
    print("  T_m is built directly from rhobar, not the physically relevant")
    print("  closure). Addendum #3's own already-established on-shell")
    print("  reduction (div_Tm_1, using rhobar's uncoupled relation, divided")
    print("  by rhobar) IS exactly V_x_dot+2*H*V_x -- reused directly, not")
    print("  recomputed:")
    matter_euler_bare_p56 = rhobar * (sp.diff(Vx, t) + 2 * Hubble * Vx)
    check_euler_tie_in = sp.simplify(
        div_Tm_1.subs(sp.diff(rhobar, t), rhobar_decoupled_sub) - matter_euler_bare_p56
    )
    assert check_euler_tie_in == 0, (
        "div_Tm_1 on-shell does not reduce to rhobar*(V_x_dot+2*H*V_x) -- "
        "Addendum #3's own already-established reduction is inconsistent, "
        "do not report Addendum #4's tie-in"
    )
    print("    div_Tm_1 (on-shell) == rhobar*(V_x_dot+2*H*V_x)  [CONFIRMED,")
    print("    matches Addendum #3's own reduction exactly]")
    print("  FINDING_P59's own Route-B comparison (not reproduced here, same")
    print("  reason as nu=0) found this quantity does NOT simply factor out")
    print("  of [div_Tm_1(rho_phys)-Q^1_A] the way div_Tm_0 did for nu=0 -- a")
    print("  genuine extra term survives in the raw V_x variable. The exact")
    print("  corrected form, in W_x:=(1-g_hat*phibar)*V_x (=rho_phys*V_x/")
    print("  rhobar, momentum-per-bare-mass -- FINDING_P59 Part 4-5):")
    Wx = sp.Function("W_x")(t, x, y, z)
    corrected_euler_lhs = sp.diff(Wx, t) + 2 * Hubble * Wx
    corrected_euler_rhs = ghat * sp.diff(deltaphi, x) / a**2
    print(f"    {corrected_euler_lhs} = {corrected_euler_rhs}")
    print("  Consistency check available from THIS file's own quantities: the")
    print("  RHS above must equal Q^1/rhobar (Part 2's own already-verified")
    print("  Q^1, already shown to cancel rhobar exactly earlier in this")
    print("  file's own Addendum #3):")
    check_addendum4_rhs = sp.simplify(corrected_euler_rhs - Q1 / rhobar)
    assert check_addendum4_rhs == 0, (
        "Addendum #4's W_x-equation RHS does not match Q^1/rhobar -- "
        "propagation error, do not report this correction"
    )
    print("  -> CONFIRMED: RHS matches Q^1/rhobar exactly -- the SAME Q^1")
    print("     this file already derived in Part 2, tying the correction")
    print("     back to this file's own already-verified quantity, not a")
    print("     free-floating new claim. The LHS's own W_x-rescaling")
    print("     derivation (why the 1/(1-g_hat*phibar) factor cancels) is")
    print("     FINDING_P59's own Part 5 result, NOT re-derived here -- it")
    print("     requires the rho_phys tensor construction only P59 performs;")
    print("     re-running that machinery here would duplicate P59's own")
    print("     already-skeptic-reviewed work for no new epistemic content.")
    print()
    print("  Pre-answered skeptic concerns (per the Falsification Ladder's own")
    print("  SKEPTIC-PRE-ANSWERED shortcut -- this addendum is propagation of")
    print("  FINDING_P59's own already-skeptic-reviewed result into P56's own")
    print("  equations, not new tensor algebra; both checks above are genuine,")
    print("  reused directly from this file's own already-computed div_Tm_0/")
    print("  div_Tm_1/Q^1, not asserted):")
    print("  (1) rhobar/deltarho HERE = rhobar_A/delta_rho_A in P59's naming --")
    print("      established by FINDING_P58's own retraction, not new here.")
    print("  (2) Part 4/Part 5's ORIGINAL equations are not 'wrong' on their")
    print("      own premise (T_m built directly from rhobar) -- RELABELED")
    print("      Route-A-only, superseded for physical/downstream use by the")
    print("      corrected forms above, matching FINDING_P59's own Verdict")
    print("      wording exactly ('correct only if T_m is built from rho_A")
    print("      directly').")
    print("  (3) No double-counting risk: only the corrected (rho_phys-")
    print("      closure) forms above should be used downstream (e.g.")
    print("      FINDING_P50A's Poisson relation) -- stated explicitly here,")
    print("      not left ambiguous.")
    return 0
