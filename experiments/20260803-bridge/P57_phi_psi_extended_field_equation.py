...
def main():
    ...
    print("\n[Added after skeptic review, Step 8a] Four scope caveats named")
    print("explicitly, per the skeptic's own findings -- none are computational")
    print("errors (skeptic independently re-derived the full result by hand via")
    print("a DIFFERENT method -- divergence form of box(phi), cross-checked")
    print("against the standard Mukhanov-style Newtonian-gauge KG equation --")
    print("and confirmed term-for-term, including the non-trivial Psi-source")
    print("cancellations):")
    print("  KG-B1b (matching FINDING_P54's own gauge label): same conformal-")
    print("  Newtonian gauge as P46-P54 throughout -- g_0i=0, no vector/tensor")
    print("  modes -- stated explicitly here, was implicit-only before.")
    print("  ASSUMPTION, not derived: delta_rho is taken at the SAME")
    print("  perturbative order eps as Phi, Psi, delta_phi (not suppressed,")
    print("  e.g. by isocurvature). If delta_rho were O(eps^2), the -g_hat*")
    print("  delta_rho term would drop from this eps^1 equation entirely.")
    print("  NOT YET CLOSED: Phi and Psi are kept independent here (no")
    print("  anisotropic-stress/Einstein-constraint relating them imposed) --")
    print("  this equation alone does not predict delta_phi without a")
    print("  companion Einstein equation (the B1<->B2 compatibility check,")
    print("  still not attempted).")
    print("  rho treated as an external, bookkept scalar (NOT rho(phi)) --")
    print("  the SAME closure-condition caveat already named in P55/P56,")
    print("  now inherited here unchanged, not newly discovered.")
    ...
    print("  [Added after skeptic review] this simplification is algebraically")
    print("  SHALLOW -- a one-line consequence of substituting the already-known")
    print("  background equation, not new physics content. Reported as")
    print("  bookkeeping, not as an independent check.")
