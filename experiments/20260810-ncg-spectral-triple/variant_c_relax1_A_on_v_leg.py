"""RELAXATION MAP item 1 of variant_c_full_order_zero_first_order.py's own
claim.md entry: change ONE assumption -- A acts on the v-LEG instead of the
s-leg, J UNCHANGED (still swap(s,c)+conj). Tests whether the Y_gen=0 kill
found for "A on s" is a property of the CONSTRUCTION CLASS (one-leg trick +
swap(s,c) J, regardless of WHICH leg A sits on) or specific to the s-leg
choice. Everything else copied verbatim from that file; only embed_s_leg's
block offset changed from S0 to V0, per Minimal Relaxation Rule (one change).
...
def embed_s_leg(M8: np.ndarray) -> np.ndarray:
    """RELAXED: acts as M8 on the v-block (indices 0:8), identity on s and c
    (name kept as embed_s_leg for a minimal diff against the base file --
    the body now uses V0, not S0)."""
    M = np.eye(dim_gen, dtype=complex)
    M[V0 : V0 + 8, V0 : V0 + 8] = M8
    return M
