"""RELAXATION MAP item 3 of variant_c_full_order_zero_first_order.py's own
claim.md entry: change ONE assumption -- A acts DIAGONALLY on ALL THREE legs
simultaneously (embed_all_legs(M8) = block_diag(M8,M8,M8)), abandoning the
one-leg bimodule trick entirely, J UNCHANGED (still swap(s,c)+conj). This is
the genuinely distinct ansatz in the Relaxation Map -- items 1 and 2 (which
leg is the spectator / which pair J swaps) are related to the already-run
base case by the su(3)-diagonal + U-triality symmetry already established
(su(3) acts identically per leg, U cyclically permutes v/s/c), so they are
NOT independent tests of the base result; this one is, because it removes
the one-leg trick's core assumption rather than relabelling roles under it.
Prediction stated BEFORE running (Minimal Relaxation Rule / AOG-1
pre-registration): order-zero is expected to FAIL immediately, the same way
main_experiment_A_Dfull_triality.py's FIRST (buggy) representation attempt
failed with block_diag(M,conj(M)) on both legs of the killed branch -- a
non-abelian A acting identically on multiple legs at once generically
collides with J's right-action landing on the SAME legs. Recorded honestly
whether this prediction holds or not.
...
def embed_s_leg(M8: np.ndarray) -> np.ndarray:
    """RELAXED: acts as M8 on ALL THREE legs simultaneously (block-diagonal),
    abandoning the one-leg trick (name kept as embed_s_leg for a minimal
    diff against the base file)."""
    M = np.eye(dim_gen, dtype=complex)
    for offset in (V0, S0, C0):
        M[offset : offset + 8, offset : offset + 8] = M8
    return M
