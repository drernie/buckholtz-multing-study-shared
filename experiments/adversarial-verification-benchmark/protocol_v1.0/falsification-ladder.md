...
**Paraphrase-Sensitivity Probe (part of Step 8a, high-stakes PROMOTE claims only):**
A single Skeptic invocation is one judge call, and judge verdicts can be sensitive to
how the falsification request is worded, independent of the claim or code being
reviewed — this is not hypothetical (JudgeSense, arXiv:2604.23478, cited as Tier1 in
the 2026-08-24 Falsification Audit; independently reproduced the same session on this
stack's own code — `Claude-cod-top-2026` experiment
`20260824-elai-hooks-skeptic-pilot`, two Skeptic invocations on identical claim+code,
formal vs. paraphrased prompt, returned `[WEAKENED]` vs `[FALSIFIED]` — only the
paraphrased variant surfaced a real, independently-confirmed bug).

For a claim about to be promoted from a Full-Ladder experiment: run Skeptic Step 8a
**twice**, same claim.md + same code, context-asymmetric both times (no session
history), but reword the falsification-request prompt between the two runs (different
register/sentence structure, identical semantic content — not a different claim).

| Both runs agree | Runs disagree |
|---|---|
| Proceed per the single-run Response Matrix above | Do NOT average or pick the more convenient verdict. Treat the disagreement itself as a finding: log both verdicts + both prompts in `decision.md`, and default to the MORE SKEPTICAL of the two (skeptic-leaning tie-breaker, per `research-methodology.md`'s own default) until the disagreement is resolved by a third, differently-prompted run or by manual review of which run's reasoning actually traced the code vs. asserted from the spec. |

**Cost discipline (do not run on every Skeptic invocation):** two Skeptic calls cost
2x one. Reserve this probe for claims where a wrong PROMOTE is expensive — Full-Ladder
promotion, security/auth/schema changes, or any claim already carrying a `[WEAK]`
marker from the Response Matrix above. Do not apply it reflexively to Micro/Standard-
Ladder claims; that is the same disproportionate-cost failure mode this rule file's
own Skeptic Response Matrix already warns against elsewhere.
...
**Named precedent for "Different model"/"Blind replication" tiers, even informally
(2026-09-01, methodology-comparison audit against Anthropic's Riemann-zeta project):**
their headline result was read by external human mathematicians (Brian Conrey, Dan
Goldston) at least once before being called finished — a cheap, informal instance of
the "Blind replication by another group" tier, not a full peer-review cycle. This
repo's own research projects (H-12, N-7, Buckholtz) have never invoked any tier above
"Same model, isolated context" (the FL skeptic) on a promoted result. The gap is not
that this rule doesn't exist — it has covered this exact case since it was written —
it is that nothing has ever prompted actually reaching for it. Before calling a
headline result "promoted," ask whether a cheap instance of a higher rung (a different
model, a second independently-written implementation, or an informal sanity-check from
someone outside the session) was actually tried, not just theoretically available.
...
**Caveat Gate (added 2026-08-30, source: Buckholtz/MULTING audit project — a scope caveat
in that project's own `docs/127` named, verbatim, "a genuinely different closure... could
reintroduce q-dependence; non-uniqueness for arbitrary closures is NOT settled" — the
project's own C1 vector-orientation *sub-test* was pearled with a `next_check`, but this
broader, named alternative was not, and sat as unexamined prose for over a month until the
paper's own author independently published a different closure mechanism that did exactly
what the caveat had already flagged as possible. Cheap, general fix, not specific to that
one incident.) Run whenever you write a scope-caveat / "what this does NOT establish"
section — symmetric to the Pearl Gate, opposite trigger:** the Pearl Gate fires on an
unexpected *result*; this one fires on a *named, untested alternative you just wrote down
yourself and then walked away from*. Ask: "does this caveat name a SPECIFIC alternative
construction, mechanism, or closure — not a vague 'more work needed', but something you
could point at and build — that has NOT been attempted?" If yes, it does not get to live
only as prose in one file's own caveat section — add it to `pearl_registry/INDEX.md` with
the same row shape as above (`observation` = the named alternative; `falsifiable_prediction`
= what would change if it were built; `trigger_condition` = what would justify picking it
up). If no `next_check` date is warranted yet (the alternative is real but not yet worth
scheduling), still add the row — a Caveat Gate entry with a distant or provisional
`next_check` beats no entry at all, per the Pearl Registry's own hard rule below.

**Why this gate exists, not just the Pearl Gate:** a caveat sentence like *"a genuinely
different [approach] could give a different answer; this has not been settled"* is not a
hedge — it is a testable claim about the existence of an untested option, sitting right next
to the claim you DID test. Left as prose, it is real but invisible to any later review that
doesn't happen to re-read that one file; a project can accumulate dozens of these across
`caveats.md`/`FINDING_*.md` files with no mechanism ever re-surfacing them, and the gap can
sit unexamined for months — right up until an outside party independently builds the
alternative and gets a different answer, at which point the caveat that predicted this was
possible was already on record and simply never acted on. The Caveat Gate is the cheap fix:
route it through the SAME ledger the Pearl Gate already uses, so it inherits the same
`next_check` re-surfacing mechanism (`hooks/research_health_loop.py`) instead of decaying
into unread prose.
...
High-stakes PROMOTE?     → Paraphrase-Sensitivity Probe (part of Step 8a): run Skeptic twice,
                           reworded prompt, same claim+code. Disagreement → log both, default
                           to the more skeptical verdict. Not for Micro/Standard — cost discipline.
...
Wrote a scope-caveat / "does NOT establish" naming a SPECIFIC untested
alternative?             → Caveat Gate: pearl_registry/INDEX.md row, same shape as Pearl
                            Gate. Don't let it live only as prose in one file — that's how a
                            named, real alternative sits unexamined for months.
