...
of $\sim$80--380) applies to a thermalized mirror sector only;
...
$N_\mathrm{eff} = 2.99 \pm 0.17$) is exceeded by a factor of $\sim$80--380
...
Quantitatively, the two escapes are mutually exclusive: matching
$\omega_\mathrm{DM}/\omega_b = 5.369$ via five isomer sectors requires
per-sector number densities equal to the baryonic one to $\sim$1.2\%
($n_i/n_b = 1.000 \pm 0.012$), which only a shared thermal history could
plausibly enforce --- yet any thermal history dense enough to fix
$n_i \approx n_b$ over-produces $\Delta N_\mathrm{eff}$ by the factors above,
while the non-thermal (grav-only) history that satisfies
$\Delta N_\mathrm{eff}$ leaves the 5:1 ratio underived.  The
$\Delta N_\mathrm{eff}$ escape and the 5:1 abundance argument therefore
cannot both be claimed within the same thermal history.
\reconstruction
...
exceeding the Planck bound by a factor of $\sim$380; a gravitational-only sector (IDM as stated) gives
