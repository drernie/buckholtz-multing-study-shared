...
\emph{more} ICM thermal energy show a \emph{smaller} mass gap. We tested two
standard-physics explanations for this partial correlation and both fail to
account for it: splitting the sample at the median $M_\mathrm{WL}$ leaves the
partial correlation unchanged in both subsamples ($r=-0.732$ vs.\ $r=-0.731$,
Fisher $z=0.005$, $p=0.50$, ruling out a mass-threshold effect), and
controlling additionally for X-ray centroid shift $w_X$ (a standard cluster
...
dynamical-state confounding. The partial correlation is therefore robust
against two natural standard-physics alternative explanations, yet it does not
constitute evidence for the preprint's mechanism: it is a cluster-interior
(ICM) result, not the cosmic-web filament (WHIM) effect the mechanism
proposes, and no causal model for the partial correlation itself is offered
here. We label this an open, reproducible empirical pattern, not a validation.
Crucially, the preprint's mechanism refers to warm-hot intergalactic medium
(WHIM, $T\sim 10^5$--$10^7$~K) in cosmic-web \emph{filaments} rather than
the cluster ICM; the broader hypothesis remains inconclusive pending
WHIM-specific observations.
...
zero-parameter Newtonian baseline. Two explanations remain open and are not
distinguished by this test: (a) the quadrupole structure is genuinely
disfavored by cluster data at these scales, or (b) the $Y_\mathrm{SZ}$- and
$\sigma_v$-derived $T_i$ proxies used here are intrinsically noisier
tracers of the cosmic expansion trend than $M_{500}$ alone, so that any
nonlinear combination involving them --- MULTING-specific or not --- would
underperform mass-only scaling regardless of whether MULTING is correct.
Resolving this requires either a volume-limited sample with uniformly
high-quality $T_X$ across the full redshift range, or an independent
proxy-noise characterization not attempted here.
\reconstruction
...
\hline
$m_\tau^\mathrm{exact} = 1776.840$~MeV if Eq.~(32) exact ($-1.00\sigma$ from PDG) &
...
\hline
Eq.~31 (7:9:17 bosons) & $\triangle$ $m_W/m_H$ at $0.04\%$; $1.14\sigma$ (H), $3.8\sigma$ (W) & PDG 2024 & $W$-mass world average (LHC Run~3) is the discriminant \\
Eq.~32 (fermion-gravity) & $\checkmark$ $1.0\sigma$ & PDG 2024 & Belle~II $m_\tau$ (2027) sharpens exact vs.\ approximate \\
...
IDM $N=5$ equal-mass & $\triangle$ $5.67\sigma$ if equal & Planck 2018 (EXP-N) & relax to unequal isomer masses (next row) \\
...
$f\sigma_8$ dipole proxy & $\triangle$ $r{=}0.851$ ($n{=}10$, $z{\le}5$) & Table A1 $+$ $\Lambda$CDM & DESI DR1 $+$ author's $\beta_d$ \\
Secondary $z{=}8.5$ bump & $\triangle$ structural & Two-component $\varepsilon$ & quadrupole $\beta_q$ from author \\
