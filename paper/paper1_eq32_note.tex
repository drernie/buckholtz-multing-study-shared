...
\emph{present} PDG central value $m_\tau=1776.86$~MeV already sits at
$2.0\,\sigma_{\mathrm{Belle\,II}}$ from Eq.~(\ref{eq:main}); a Belle~II
measurement consistent with the current central value, at its anticipated
precision, would in fact mildly disfavor the exact relation, while a shift
toward $1776.840$~MeV would confirm it.
...
We note one further asymmetry between the two relations, not as grounds for
dismissing Eq.~(\ref{eq:tau-alt}) but as a genuine difference in how testable
each is: Eq.~(\ref{eq:tau-alt}) depends on the top-quark mass raised to the
18th power, and the top-quark mass, unlike the lepton masses entering
Eq.~(\ref{eq:main}), carries an intrinsic renormalization-scheme ambiguity of
order several GeV between the pole-mass and $\overline{\mathrm{MS}}$
conventions. Using the PDG~2024 pole mass, $M_t=172.76$~GeV, gives the quoted
$0.0070\%$; using the PDG~2024 $\overline{\mathrm{MS}}$ mass,
$\overline{m}_t(\overline{m}_t)=162.77$~GeV~\cite{pdg2024}, in its place
raises the error to $65.8\%$---a factor of roughly $9\times 10^3$
worse---because an $18$th power amplifies an $O(6\%)$ mass shift into an
$O(1)$ change in the predicted ratio. Eq.~(\ref{eq:main}) has no analogous
exposure: $m_e$ and $m_\tau$ are on-shell lepton masses with no comparable
scheme freedom. We read this as evidence that Eq.~(\ref{eq:tau-alt}) is the
more fragile of the two coincidences, not as a reason to discount it
entirely, since a preferred scheme (the pole mass) is the conventional choice
in this context.
...
measurement to $0.0135\%$, that is among the two most accurate members of a
large, honestly pooled family of comparable relations (Sec.~\ref{sec:refmass}),
and whose coefficients admit an exceptional-algebra reading whose origin
remains to be explained. Whether the relation reflects a
