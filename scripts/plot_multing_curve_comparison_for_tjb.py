"""Comparison figure attached to the 2026-08-03 letter to Dr. Buckholtz.

Shows exactly the two points the letter makes in prose, nothing else:
  1. the orange curve has a shallow minimum near z~0.10, a feature the
     flat LCDM reference curve does not have anywhere in the plotted range;
  2. the orange curve and Table A1's H_MULT column diverge, growing to
     roughly 11/25/36/44% at z=0.65/1.0/1.5/2.1, the last point past the
     source figure's own marked CC-calibration boundary at z=1.965.

Uses only the curves in the supplied figure and the Table A1 values --
no external dataset, so there is no unmatched-point ambiguity to explain.

The blue LCDM curve is fit first as a positive control (known target,
recovered to 0.002% rms) so the reader can see the extraction is trustworthy
before looking at what it finds on the unknown curve.

Reads   : the emailed figure PDF (curve geometry, selected by colour)
          data/table_a1_source_verified.csv
Writes  : reports/multing_curve_comparison_for_tjb.png  (and .pdf)
...
OUT = REPO / "reports" / "multing_curve_comparison_for_tjb"
...
def main() -> int:
    ...
    # --- feature 1: the low-z minimum, on the curve as drawn (no smoothing) ---
    lowz = np.linspace(max(zo.min(), -0.15), 0.6, 4000)
    lowz_h = np.interp(lowz, zo, ho)
    i_min = int(np.argmin(lowz_h))
    z_min, h_min = lowz[i_min], lowz_h[i_min]
    print(f"FEATURE  shallow minimum near z={z_min:.2f}, H={h_min:.1f}")
    ...
    ax.plot(
        ...
        color="#8c1d1d",
        zorder=5,
        label="Table A1 $H_{\\rm MULT}$ values",
    )

    ax.plot([z_min], [h_min], "v", ms=10, color="#d95923", mec="k", mew=0.8, zorder=6)
    ax.annotate(
        f"shallow minimum near $z\\approx{z_min:.2f}$",
        xy=(z_min, h_min),
        xytext=(0.30, 0.62),
    ...
    ax.set_title(
        "Two quantitative features of the current MULTING figure\n"
        "A shallow low-z minimum, and a numerical distinction from Table A1",
        fontsize=11.0,
        pad=12,
    )
    ax.text(
    ...
    bx.plot(
        ...
        color="#8c1d1d",
        lw=1.5,
        label="orange curve vs Table A1 $H_{\\rm MULT}$  (21 % rms)",
    )
    ...
    fig.text(
        0.5,
        0.008,
        "Curves digitised by colour from your figure. Squares are Table A1 "
        "$H_{\\rm MULT}$ values, not a fit.\nThe $z=2.1$ point lies beyond the "
        "figure's own marked CC-calibration boundary at $z=1.965$.",
        ha="center",
        fontsize=8,
        color="0.35",
    )
