...
def test_row_permutation(z, h_data, beta_d, beta_q, n=100, seed=42):
    ...
    chi2_shuf = [
        compute_fit_quality(np.random.permutation(z), h_data, beta_d, beta_q) for _ in range(n)
    ]
...
def test_randomised_beta(z, h_data, beta_d, beta_q, n=100, seed=42):
    ...
    chi2_rand = [
        compute_fit_quality(
            z, h_data, np.random.uniform(*BETA_D_RANGE), np.random.uniform(*BETA_Q_RANGE)
        )
        for _ in range(n)
    ]
...
def main():
    ...
    overall = (
        "FAIL"
        if any(x.verdict == "FAIL" for x in [r1, r2, r3])
        else "PASS"
        if all(x.verdict == "PASS" for x in [r1, r2, r3])
        else "WARN"
    )
